...
func _input(event):
#	if event extends InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		var from = get_node("cambase/Camera3D").project_ray_origin(event.position)
		var to = from + get_node("cambase/Camera3D").project_ray_normal(event.position)*100
		var p = get_closest_point_to_segment(from, to)
		
		begin = get_closest_point(get_node("robot_base").get_position())
		end = p
